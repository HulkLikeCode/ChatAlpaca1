...
def render_header() -> None:
    ...
    st.caption(
        f"Up to {MAX_PORTFOLIOS} internally tracked portfolios · exact holdings · buy-and-hold benchmarks · Alpaca order allocation"
    )
...
def render_portfolio_admin(portfolios: list[Portfolio]) -> None:
    ...
    if created:
        try:
            ...
            st.session_state.flash = f"Portfolio '{portfolio.name}' added. Record or import holdings to display it."
            st.rerun()
        except Exception as exc:
